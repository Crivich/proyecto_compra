...
class Pedido{
    ...
    public function registrarDetalle($_params){
        $sql = "INSERT INTO `detalle_pedidos`(`pedido_id`, `plato_id`, `precio`, `cantidad`) 
        VALUES (:pedido_id,:plato_id,:precio,:cantidad)";

        $resultado = $this->cn->prepare($sql);

        $_array = array(
            ":pedido_id" => $_params['pedido_id'],
            ":plato_id" => $_params['plato_id'],
    ...
    public function mostrarDetallePorIdPedido($id)
    {
        $sql = "SELECT 
                ...
                INNER JOIN platos pe ON pe.id= dp.plato_id
